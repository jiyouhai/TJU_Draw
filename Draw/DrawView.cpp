...
BEGIN_MESSAGE_MAP(CDrawView, CView)
	...
	ON_COMMAND(ID_RIGHT, &CDrawView::OnRight)
	ON_COMMAND(ID_CIRCLE, &CDrawView::OnCircle)
	ON_COMMAND(ID_SQUARE, &CDrawView::OnSquare)
	ON_COMMAND(ID_32809, &CDrawView::On32809)
	ON_COMMAND(IDM_MOVE1, &CDrawView::OnMove1)
	ON_COMMAND(IDM_RATIO, &CDrawView::OnRatio)
END_MESSAGE_MAP()
	...
	for (int i = 0; i < m_ptrArray.GetSize(); i++)//重绘
	{
		...
		if (!m_isRedraw)//判断是否已发生一次重绘
		{
			if (m_reDraw == TRUE)
			{
				...
				if (((CGraphic*)m_ptrArray.GetAt(i))->m_nDrawType != 6&& ((CGraphic*)m_ptrArray.GetAt(i))->m_nDrawType != 15&& ((CGraphic*)m_ptrArray.GetAt(i))->m_nDrawType != 16&& ((CGraphic*)m_ptrArray.GetAt(i))->m_nDrawType != 100)//填充判定矩形和多边形
				...
				if (m_pixelColor != dc.GetPixel(m_ptMove1))//按顺序重绘到点击的图形颜色是否改变
				{
					...
					if (((CGraphic*)m_ptrArray.GetAt(i))->m_nDrawType != 6 && ((CGraphic*)m_ptrArray.GetAt(i))->m_nDrawType != 102)//填充判定矩形和多边形
					{
						pDC->FillRect(&rc, &Brush);
					}
					else
...
void CDrawView::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if (m_nDrawType == 2 || m_nDrawType == 3 || m_nDrawType == 4 || m_nDrawType == 6 || m_nDrawType == 15 || m_nDrawType == 16 || m_nDrawType == 100 || m_nDrawType == 101 || m_nDrawType == 102)//旋转后绘制图形时判断条件更改
		m_isSpin = FALSE;
	...
	if (m_nDrawType == 8 || m_nDrawType == 9 ||m_nDrawType==20)
	{
		...
		if (m_isSelect)
		{
			...
			//判断是否点击在九个定位小正方形内
			flag = (rc1.PtInRect(point) || rc2.PtInRect(point) || rc3.PtInRect(point)
				|| rc4.PtInRect(point) || rc5.PtInRect(point) || rc6.PtInRect(point)
				|| rc7.PtInRect(point) || rc8.PtInRect(point) || rc9.PtInRect(point));
			if (flag)
			{
				//m_isSelect=TRUE;
...
void CDrawView::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	case 15://绘制等腰
		...
		dc.SetROP2(R2_MASKPEN);
		{
			...
			dc.MoveTo(pointding);
			dc.LineTo(pointd2);
			m_ptMid1.x = (pointd1.x + pointd2.x) / 2;//三角形边界上六个点的关系
			m_ptMid1.y = (pointd1.y + pointd2.y) / 2;
			m_ptMid2.x = (pointd1.x + pointding.x) / 2;
			m_ptMid2.y = (pointd1.y + pointding.y) / 2;
			m_ptMid3.x = (pointd2.x + pointding.x) / 2;
			m_ptMid3.y = (pointd2.y + pointding.y) / 2;
			{
				CGraphic* cgraphic = new CGraphic(6, m_ptMid1, m_ptMid2, m_ptMid3, pointd1, pointd2,
					pointding, NULL, NULL, NULL,
	...
	case 100://绘制直角三角形
		...
		dc.SetROP2(R2_MASKPEN);
		{
			CPoint m_ptMiddle11, m_ptOrigin1;
			m_ptMiddle11.y = m_ptOrigin.y;
			m_ptOrigin1.x = m_ptOrigin.x;
			m_ptOrigin1.y = point.y;
			m_ptMiddle11.x = m_ptOrigin.x;
			dc.MoveTo(m_ptOrigin1);
			dc.LineTo(point);
			dc.MoveTo(m_ptOrigin1);
			dc.LineTo(m_ptMiddle11);
			dc.MoveTo(m_ptMiddle11);
			dc.LineTo(point);
			m_ptMid1.x = (m_ptOrigin1.x + point.x) / 2;//三角形边界上六个点的关系
			m_ptMid1.y = (m_ptOrigin1.y + point.y) / 2;
			m_ptMid2.x = (m_ptOrigin1.x + m_ptMiddle11.x) / 2;
			m_ptMid2.y = (m_ptOrigin1.y + m_ptOld2.y) / 2;
			m_ptMid3.x = (point.x + m_ptMiddle11.x) / 2;
			m_ptMid3.y = (point.y + m_ptMiddle11.y) / 2;
			{
				CGraphic* cgraphic = new CGraphic(6, m_ptMid1, m_ptMid2, m_ptMid3, m_ptOrigin1, point,
					m_ptMiddle11, NULL, NULL, NULL,
					m_ptColor, m_clr, m_clr2, m_nLineStyle, m_nLineWidth, m_xForm);
				m_ptrArray.Add(cgraphic);
			}
		}
	...
	case 101://绘制圆
		...
		{
			CGraphic* cgraphic = new CGraphic(4, m_ptOrigin, m_ptOld, m_ptOld2, m_ptOrigin2, m_ptEnd2,
				m_ptTop, m_ptBottom, m_ptLeft, m_ptRight,
				m_ptColor, m_clr, m_clr2, m_nLineStyle, m_nLineWidth, m_xForm);
			m_ptrArray.Add(cgraphic);
		}
	...
	case 102://绘制正方形
		...
		m_ptRight.y = (m_ptEnd2.y + m_ptOld.y) / 2;
		{
			CGraphic* cgraphic = new CGraphic(3, m_ptOrigin, m_ptOld, m_ptOld2, m_ptOrigin2, m_ptEnd2,
				m_ptTop, m_ptBottom, m_ptLeft, m_ptRight,
				m_ptColor, m_clr, m_clr2, m_nLineStyle, m_nLineWidth, m_xForm);
			m_ptrArray.Add(cgraphic);
		}
...
void CDrawView::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (m_bDraw == TRUE)
	{
		...
		case 15://绘制等腰
			dc.SetROP2(R2_NOTXORPEN);
			{
				CRect dengyao = CRect(m_ptOrigin, m_ptOld);
				CPoint pointd1, pointd2, pointding;
				pointd1.x = dengyao.right;
				pointd1.y = dengyao.bottom;
				pointd2.x = dengyao.left;
			...
			dc.SetROP2(R2_NOTXORPEN);
			{
					CRect dengyao1 = CRect(m_ptOrigin, m_ptOld);
					CPoint pointd11, pointd12, pointding1;
					pointd11.x = dengyao1.right;
					pointd11.y = dengyao1.bottom;
					pointd12.x = dengyao1.left;
					pointd12.y = dengyao1.bottom;
					pointding1.x = dengyao1.left + (dengyao1.right - dengyao1.left) * 0.5;
					pointding1.y = dengyao1.top;
					dc.MoveTo(pointd11);
					dc.LineTo(pointding1);
					dc.MoveTo(pointd12);
					dc.LineTo(pointd11);
					dc.MoveTo(pointding1);
					dc.LineTo(pointd12);
			}
				break;
		case 16://绘制等边
			...
			{
				m_ptOld.y = m_ptOld.x - m_ptOrigin.x + m_ptOrigin.y;
				CRect dengbian = CRect(m_ptOrigin, m_ptOld);
				CPoint pointd1, pointd2, pointding;
				pointd1.x = dengbian.right;
				pointd1.y = dengbian.bottom;
				pointd2.x = dengbian.left;
			...
			{
				m_ptOld.y = m_ptOld.x - m_ptOrigin.x + m_ptOrigin.y;
				CRect dengbian1 = CRect(m_ptOrigin, m_ptOld);
				CPoint pointd11, pointd12, pointding1;
				pointd11.x = dengbian1.right;
				pointd11.y = dengbian1.bottom;
				pointd12.x = dengbian1.left;
				pointd12.y = dengbian1.bottom;
				pointding1.x = dengbian1.left + (dengbian1.right - dengbian1.left) * 0.5;
				pointding1.y = dengbian1.bottom + (dengbian1.top - dengbian1.bottom) * 0.86602;
				dc.MoveTo(pointd11);
				dc.LineTo(pointding1);
				dc.MoveTo(pointd12);
				dc.LineTo(pointd11);
		...
		case 8:	//平移及放大缩小
			if (m_isSelect)
			{
				...
				case 4:
					...
					break;
				case 6:
					m_center.x = (((CGraphic*)m_ptrArray.GetAt(num))->point4.x + ((CGraphic*)m_ptrArray.GetAt(num))->point5.x+ ((CGraphic*)m_ptrArray.GetAt(num))->point6.x) / 3;
					m_center.y = (((CGraphic*)m_ptrArray.GetAt(num))->point4.y + ((CGraphic*)m_ptrArray.GetAt(num))->point5.y + ((CGraphic*)m_ptrArray.GetAt(num))->point6.y) / 3;
					long distance1, distance2;
					distance1 = long(sqrt((m_ptMove1.x - m_center.x) * (m_ptMove1.x - m_center.x) + (m_ptMove1.y - m_center.y) * (m_ptMove1.y - m_center.y)));
					distance2 = long(sqrt((m_ptMove2.x - m_center.x) * (m_ptMove2.x - m_center.x) + (m_ptMove2.y - m_center.y) * (m_ptMove2.y - m_center.y)));
					vector1.x = (((CGraphic*)m_ptrArray.GetAt(num))->point4.x - m_center.x) * distance2 / distance1;
					vector1.y = (((CGraphic*)m_ptrArray.GetAt(num))->point4.y - m_center.y) * distance2 / distance1;
					vector2.x = (((CGraphic*)m_ptrArray.GetAt(num))->point5.x - m_center.x) * distance2 / distance1;
					vector2.y = (((CGraphic*)m_ptrArray.GetAt(num))->point5.y - m_center.y) * distance2 / distance1;
					vector3.x = (((CGraphic*)m_ptrArray.GetAt(num))->point6.x - m_center.x) * distance2 / distance1;
					vector3.y = (((CGraphic*)m_ptrArray.GetAt(num))->point6.y - m_center.y) * distance2 / distance1;
					if (!flag)//三角形的平移
					{
						if (m_isSpin)
						{
					...
					else 
					{
						...
						TriangleRelation();
						((CGraphic*)m_ptrArray.GetAt(num))->point4 = m_center + vector1;
						((CGraphic*)m_ptrArray.GetAt(num))->point5 = m_center + vector2;
						((CGraphic*)m_ptrArray.GetAt(num))->point6 = m_center + vector3;
						DrawTriangle();
